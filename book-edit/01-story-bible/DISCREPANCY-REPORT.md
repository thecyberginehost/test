...
- **✅ Q2 RESOLVED — `Val'Kora` = `Vor'kara`:** confirmed the **same word**. `Vor'kara` is a **formal greeting of respect** (said to those who command respect, reciprocated in kind) — and it's exactly what Junior's outdated radio intercepts in **Ch01** (the intercepted "voices" = Obsidian/Void transmissions bleeding through). **Action:** on Ch01's edit pass, standardize **`Val'Kora` → `Vor'kara`**. (The Ch03 "`Dra'Kor Vhel'Tan`" chant is a *different* intercepted phrase, also bleeding through — keep, it's the same seeding thread.)
